...
echo.
echo 创建输出目录...
if not exist "%OUTPUT_ROOT%\Models" mkdir "%OUTPUT_ROOT%\Models"
if not exist "%OUTPUT_ROOT%\Json" mkdir "%OUTPUT_ROOT%\Json"
...
REM 创建Unity目录（如果不存在）
if not exist "%OUTPUT_CSHARP%" mkdir "%OUTPUT_CSHARP%"
